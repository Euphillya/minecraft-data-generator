...
public class EntitiesDataGenerator implements IDataGenerator {
    public static JsonObject generateEntity(final Registry<EntityType<?>> entityRegistry, final EntityType<?> entityType) {
        final JsonObject entityDesc = new JsonObject();
        final Identifier registryKey = entityRegistry.getKey(entityType);
        final int entityRawId = entityRegistry.getId(entityType);
        ...
        final MinecraftServer minecraftServer = DGU.getCurrentlyRunningServer();

        if (minecraftServer != null) {
            final Entity entityObject = entityType.create(minecraftServer.overworld(), EntitySpawnReason.NATURAL);
    ...
    private static String getCategoryFrom(final EntityType<?> entityType) {
        if (entityType == EntityTypes.PLAYER) return "UNKNOWN";
        final Entity entity = entityType.create(DGU.getWorld(), EntitySpawnReason.NATURAL);
        if (entity == null)
            throw new Error("Entity was null after trying to create a: " + DGU.translateText(entityType.getDescriptionId()));
        entity.discard();
        final String packageName = entity.getClass().getPackageName();
    ...
    private static String getEntityTypeForClass(final Class<? extends Entity> entityClass) {
    ...
    public JsonArray generateDataJson() {
        ...
        final Registry<EntityType<?>> entityTypeRegistry = DGU.getWorld().registryAccess().lookupOrThrow(Registries.ENTITY_TYPE);
